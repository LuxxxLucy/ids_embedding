...
def parse_args_and_config():
    ...
    parser.add_argument(
        "--runner", type=str, default="ToyRunner", help="The runner to execute"
    )
    parser.add_argument(
        "--config", type=str, default="toy.yml", help="Path to the config file"
    )
    ...
    if not args.test:
        ...
        formatter = logging.Formatter(
            "%(levelname)-8s - %(filename)-15s - %(asctime)s - %(message)s"
        )
    ...
    if not os.path.exists(new_config.storage_path):
        os.makedirs(new_config.storage_path)
    # add device
